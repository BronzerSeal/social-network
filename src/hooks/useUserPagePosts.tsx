"use client";
import { getInfinityPostsByUserId } from "@/actions/posts/getInfinityPostsByUserId";
...
export const useUserPagePosts = (userId: string) => {
  const {
    data: posts,
    isLoading,
    fetchNextPage,
    ...
    Cursor
  >({
    queryKey: ["posts", "user-page"],
    queryFn: ({ pageParam }) =>
      getInfinityPostsByUserId({
        cursor: pageParam,
        userId,
      }),
